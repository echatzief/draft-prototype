    ...
    <section id="collection" className="container py-16 md:py-20">
      <div className="mb-10 flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
        <div className="space-y-2">
          ...
          <h2 className="font-display text-4xl leading-tight text-foreground md:text-5xl">A short edit with long presence.</h2>
        </div>
        <p className="max-w-md text-sm leading-6 text-muted-foreground">
          Each piece is presented like an object in a private viewing room — minimal, tactile, and ready for direct inquiry.
        </p>
      </div>
              ...
              <div className="flex flex-col justify-between gap-8 p-6 md:p-7">
                <div className="space-y-6">
                  <div className="flex items-start justify-between gap-4">
                    <div>
                      <p className="text-xs uppercase tracking-[0.28em] text-muted-foreground">{product.subtitle}</p>
                      <h3 className="mt-3 font-display text-3xl leading-tight text-foreground">{product.name}</h3>
                    </div>
                  ...
                  <dl className="grid grid-cols-2 gap-4 border-t border-border/60 pt-5 text-sm">
                    <div>
                      <dt className="text-xs uppercase tracking-[0.22em] text-muted-foreground">Color</dt>
                      <dd className="mt-2 text-foreground">{product.color}</dd>
                    </div>
                    <div>
                      <dt className="text-xs uppercase tracking-[0.22em] text-muted-foreground">Size</dt>
                      <dd className="mt-2 text-foreground">{product.size}</dd>
                    </div>
                    <div className="col-span-2">
                      <dt className="text-xs uppercase tracking-[0.22em] text-muted-foreground">Material</dt>
                      <dd className="mt-2 text-foreground">{product.material}</dd>
                    </div>
                  </dl>
                </div>
                ...
                <div className="flex items-center justify-between gap-4 border-t border-border/60 pt-5">
                  <div>
                    <p className="text-xs uppercase tracking-[0.22em] text-muted-foreground">Order value</p>
                    <p className="mt-2 font-display text-3xl text-foreground">€{product.price}</p>
                  </div>
                  <Button variant="product" onClick={() => onAddToCart(product)}>
