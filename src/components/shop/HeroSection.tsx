      ...
      <div className="container relative grid gap-14 py-14 md:py-20 lg:grid-cols-[1.05fr_0.95fr] lg:items-end lg:gap-20">
        <div className="max-w-2xl space-y-8">
          <div className="space-y-5">
            <p className="font-brand text-sm uppercase tracking-[0.38em] text-muted-foreground">
              Draft Prototype
            </p>
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <div className="flex flex-wrap gap-2 text-xs uppercase tracking-[0.18em] text-muted-foreground">
              {lines.map((line) => (
                <span
                  key={line}
                  className="rounded-full border border-border/70 bg-secondary/60 px-3 py-2"
                >
                  {line}
                </span>
              ))}
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:gap-5">
          <article className="group relative overflow-hidden rounded-2xl border border-border/60 bg-card/70 p-6 shadow-soft">
            <div
              className="pointer-events-none absolute inset-0 bg-card-highlight opacity-0 transition-opacity duration-500 group-hover:opacity-100"
              aria-hidden
            />
            ...
            <div className="mt-12 space-y-3">
              ...
              <p className="text-sm leading-6 text-muted-foreground">
                45 pieces remaining — secure yours before they're gone.
              </p>
            </div>
          </article>
